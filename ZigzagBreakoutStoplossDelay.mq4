...
input double triggerLength = 400;
input double takeProfit = 600;
input double stopLoss = 800;
...
void OnDeinit(const int reason)
{
    ...
    for (int i = totalObjects - 1; i >= 0; i--)
    {
        ...
        if (
            ...
            StringFind(objName, "tester") == -1) // Match names starting with arrowNamePrefix
        {
...
void OnTick()
{
    ...
    // Order Delay if order triggered
    if (skipBar == 0)
    {
...
void PlaceBuyLimit(double price, int tpPoints, int slPoints)
{
    ...
    if(martingale){
        lotSize = CalculateMartingaleLotsize();
    }
...
// Function to find the last closed order and check if it was profitable
double CalculateMartingaleLotsize()
{
    int totalOrders = OrdersHistoryTotal();  // Get the total number of orders in history
    double calLotSize = lotSize;

    // Loop through the history from the last order to the first
    for (int i = totalOrders - 1; i >= 0; i--)
    {
        // Select the order by position in the history
        if (OrderSelect(i, SELECT_BY_POS, MODE_HISTORY))
        {
            // Check if the order type is OP_BUY or OP_SELL
            if (OrderType() == OP_BUY || OrderType() == OP_SELL)
            {
                double profit = OrderProfit();  // Get the profit of the selected order

                // Check if the profit is greater than zero
                if (profit < 0)
                {
                    calLotSize = calLotSize * 2.5;
                }
                else
                {
                    return calLotSize;  // The last order was not profitable
                }
            }
        }
    }

    return calLotSize; // Return false if no orders found in history
}
